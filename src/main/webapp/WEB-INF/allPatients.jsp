...
<head>
	...
	<title>OceanDrill DMR: All Patients</title>
...
<body>
	<nav class="p-3 d-flex navbar navbar-expand-lg bg-success-subtle bg-gradient align-items-center justify-content-between">
		...
		<form class="d-flex" role="search" action="/search">
			<input id="searchBar" class="form-control me-2 fs-5" type="search" name="search" placeholder="Search patient name (type last name only)">
			<input type="submit" class="btn btn-outline-success fs-5 fw-semibold" value="Search">
		</form>
	...
	<div id="regForm" class="p-3 container">
		...
		<h3 class="mb-5 text-center fw-bold">ALL PATIENTS</h3>
			...
			<tbody>
				<c:forEach var="patient" items="${allPatients}">
					<tr class="text-center">
						...
						<td>${patient.id}</td>
						<td><a href="/patients/${patient.id}">${patient.lastName}, ${patient.firstName} ${patient.middleName.substring(0,1)}.</a></td>
		...
		<a href="/patients/new" class="btn btn-success bg-gradient shadow fs-5 fw-semibold">Add New Patient</a>
	</div>
